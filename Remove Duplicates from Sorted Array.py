...
if __name__ == '__main__':
    input = 'nums'
    tests = [{input: [1, 1, 2], 'expected_k': 2, 'expected_nums': [1, 2]},
             {input: [0, 0, 1, 1, 1, 2, 2, 3, 3, 4], 'expected_k': 5, 'expected_nums': [0, 1, 2, 3, 4]}]
    for test in tests:
        print(test)
        print(f'k = {remove_duplicates1(test[input])}')
        print(f'nums = {test[input]}')
